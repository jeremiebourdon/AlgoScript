...
function Play(song, duration) {
  song.connect(audiolet.output)
  setTimeout(function() {
    song.disconnect(audiolet.output);
  }, duration);
}
...
var Synth = function(audiolet) {
  ...
  this.env = new ADSREnvelope(audiolet, 1, // Gate
  1.5, // Attack
  0.2, // Decay
  0.9, // Sustain
  2); // Release
...
var frequencyPattern = new PSequence([55, 55, 98, 98, 73, 73, 98, 98], Infinity);
...
audiolet.scheduler.play(patterns, 2, function(frequency, filterLFOFrequency, gate) {
